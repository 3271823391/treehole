...
- **普通版（Plus）**：基于可持久化身份的增强体验（头像/用户名/情绪报告），对话记忆与资料可复用。
- **全能版（Pro）**：在普通版基础上提供语音克隆上传、语音输出开关与播报队列控制。

> 页面入口示例：`/treehole_plus.html`、`/treehole_pro.html`（以路由配置为准）。
